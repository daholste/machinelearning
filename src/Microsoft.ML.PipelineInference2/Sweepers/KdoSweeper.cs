...
namespace Microsoft.ML.Runtime.Sweeper.Algorithms
{
    ...
    public sealed class KdoSweeper : ISweeper
    {
        ...
        {
            AutoMlUtils.Assert(previousRuns != null && previousRuns.Length > 1);
        ...
        {
            AutoMlUtils.Assert(history.Length == weights.Length && parentIndicies.Max() < history.Length);
                ...
                if (numericParamIndices.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        foreach (int index in numericParamIndices)
                        {
                            ...
                            child[index] = (Float)PipelineInference2.Stats.SampleFromBeta(rng, alpha, beta);
                        }
                    }
                }
        ...
        {
            AutoMlUtils.Assert(weights != null && weights.Any());
            AutoMlUtils.Assert(weights.Sum() > 0);
        ...
        private double[][] SampleDiagonalCovMultivariateGaussian(int numRVs, double[] mu, double[][] diagonalCovariance)
        {
            ...
            int d = mu.Length;
            AutoMlUtils.Assert(d > 0 && diagonalCovariance.Length == d);
            for (int i = 0; i < d; i++)
            {
                AutoMlUtils.Assert(diagonalCovariance[i].Length == d);
                for (int j = 0; j < d; j++)
                {
                    AutoMlUtils.Assert((i == j && diagonalCovariance[i][j] >= 0) || diagonalCovariance[i][j] == 0);
                }
            }
